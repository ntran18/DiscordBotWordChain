...
const filePath = "src/services/game/data/dictionary.txt";
...
function addWordToDictionary(element) {
    if (isNormalWord(element)) {
        ...
        element
            .split("_")
            .forEach((w) =>
                isNormalWord(w) ? dictionaryOriginal.add(w) : console.log(w),
            );
    } else {
        console.log(element);
    }
...
for (const key in wordnet) {
    wordnet[key]["word"].forEach((element) =>
        addWordToDictionary(element.toLowerCase()),
    );
}

fs.writeFileSync(
    filePath,
    Array.from(dictionaryOriginal).sort().join(`\n`),
    (err) => {
        if (err) throw err;
